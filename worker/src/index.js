...
async function run(env) {
  ...
  const { events, stats, errors, health, warnings } = await collectAll({
  ...
  if (!events.length && errors.length) {
    await env.FEED.put(STATUS_KEY, JSON.stringify({ at, ok: false, kept: previous.length, stats, errors, warnings, health }));
  ...
  await env.FEED.put(STATUS_KEY, JSON.stringify({ at, ok: true, count: events.length, stats, errors, warnings, health }));
